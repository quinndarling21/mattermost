...
function buildProfileItems(intl: IntlShape): UserSettingsSearchItem[] {
    const ctx: TabContext = {
        tab: 'profile',
...
function buildPluginItems(
    ...
    Object.values(pluginSettings).forEach((plugin) => {
        ...
        // Tab-level entry so the plugin itself is discoverable
        items.push(item(
            ctx,
            '',
            plugin.uiName,
...
export function buildUserSettingsSearchItems(
    ...
    if (isContentProductSettings) {
        return [
            ...
            ...buildPluginItems(intl, pluginSettings),
        ];
    }

    return [
        ...buildProfileItems(intl),
...
export function getFirstPartyProductSettingSectionIds(): string[] {
    return buildUserSettingsSearchItems(
        {
