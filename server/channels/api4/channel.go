...
func updateChannel(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	case model.ChannelTypeGroup, model.ChannelTypeDirect:
		...
		if (channel.Name != "" && channel.Name != oldChannel.Name) || (channel.DisplayName != "" && channel.DisplayName != oldChannel.DisplayName) || (channel.Purpose != oldChannel.Purpose) || (channel.Emoji != "" && channel.Emoji != oldChannel.Emoji) {
	...
	// Emoji is only updated through ChannelPatch so a full-channel PUT from
	// a client that omits the field cannot blank an existing assignment.
...
func patchChannel(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	case model.ChannelTypeGroup, model.ChannelTypeDirect:
		...
		if (patch.Name != nil && *patch.Name != oldChannel.Name) || (patch.DisplayName != nil && *patch.DisplayName != oldChannel.DisplayName) || (patch.Purpose != nil && *patch.Purpose != oldChannel.Purpose) || patch.DefaultCategoryName != nil || patch.Emoji != nil {
